...
def show_security_metrics():
    ...
    with col1:
        ...
        for key, value in metrics['zero_trust_status'].items():
            st.markdown(f"<div class='metric-value'>✓ {key.replace('_', ' ').title()}: {value}</div>", 
                       unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)

        st.markdown("<div class='metric-container'>", unsafe_allow_html=True)
        st.markdown("<div class='metric-header'>Authentication Services</div>", unsafe_allow_html=True)
...
def show_login_page():
    ...
    with st.container():
        ...
        # Classification Banner
        st.markdown("<div class='classification-banner'>UNCLASSIFIED // FOR OFFICIAL USE ONLY (FOUO)</div>", 
                   unsafe_allow_html=True)
        ...
        if auth_method == "Username/Password":
            with st.form("login_form"):
                ...
                if st.session_state.last_login_time:
                    st.info(f"Last login attempt: {st.session_state.last_login_time.strftime('%Y-%m-%d %H:%M:%S')}")
