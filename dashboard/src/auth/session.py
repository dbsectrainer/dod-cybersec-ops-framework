...
def check_account_lockout():
    ...
    if st.session_state.lockout_until and datetime.now() < st.session_state.lockout_until:
        ...
        log_security_event('lockout_check', f'Attempted access during lockout period. {remaining_time} minutes remaining')
        return True
...
def handle_failed_login():
    ...
    if remaining_attempts <= 0:
        ...
        log_security_event('account_locked', f'Account locked for {LOCKOUT_DURATION} minutes due to multiple failed attempts')
        st.error(f"Maximum login attempts exceeded. Account locked for {LOCKOUT_DURATION} minutes.")
        st.stop()
    else:
        log_security_event('failed_login', f'Failed login attempt. {remaining_attempts} attempts remaining')
...
def show_system_status():
    ...
    """.format(
        SESSION_TIMEOUT,
        st.session_state.last_activity.strftime('%Y-%m-%d %H:%M:%S')
...
def get_session_info():
    ...
    return {
        ...
        'remaining_time': SESSION_TIMEOUT - 
            ...
            if 'last_activity' in st.session_state else SESSION_TIMEOUT
    }
